...
import (
	...
	Cpodcast "apigopodcast/src/apipodcast/controllers/podcast"
	// Cpodcast "apibafpodcast/controllers/dev/podcast/orderdata"
	...
	_ "apigopodcast/src/apipodcast/docs"
	// _ "apibafpodcast/docsdev"
	// _ "apibafgate/tmp"
)
...
func main() {
	...
	// LoggingService()
	// router.Use(RequestLoggersrv())

	// v1 := router.Group("/api/v1/bafgate")
	url := ginSwagger.URL(host + addr + "/api/v1/apipodcast/swagger/doc.json") // The url pointing to API definition

	v1 := router.Group("/api/v1/apipodcast")
	{
		//Modul podcast

		v1.POST("podcast/datapodcast/list", Cpodcast.GetPodcast)
		v1.POST("podcast/datapodcast/insert", Cpodcast.InsertPodcast)
		v1.POST("podcast/datapodcast/delete", Cpodcast.DeletePodcast)
		v1.POST("podcast/datapodcast/update", Cpodcast.UpdatePodcast)
		v1.GET("/status", Cpodcast.CheckReadiness)
	...
	go func() {
		<-quit
		logger.Println("Server apipodcast is shutting down...")
		...
		if err := server.Shutdown(ctx); err != nil {
			logger.Fatalf("Could not gracefully shutdown the server apibafpodcast: %v\n", err)
		}
		close(done)
	}()

	logger.Println("Server apipodcast is ready to handle requests at", subhost, addr)
	...
	<-done
	logger.Println("Server apipodcast stopped")
...
func LoggingActivity() {
	...
	var filename string = "src/apilogpodcast/logpodcast/log" + date + ".log"
